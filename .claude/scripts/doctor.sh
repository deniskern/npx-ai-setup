...
if [ "$context_found" -eq 3 ]; then
  ...
  add_row "$WARN" "Context files"       "${context_found}/3 present — run /context-full to regenerate"
else
  add_row "$WARN" "Context files"       "None found in .agents/context/ — run /context-full"
fi
