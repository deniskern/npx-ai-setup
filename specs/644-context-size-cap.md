...
> **Spec ID**: 644 | **Created**: 2026-04-19 | **Status**: draft | **Complexity**: small | **Branch**: —
...
## Steps
- [ ] Step 1: Cap-Config als `lib/data/context-caps.json` — {file: max_lines} Mapping mit sinnvollen Defaults
- [ ] Step 2: `lib/context-size-check.sh` — liest Caps, prüft alle Files in `.agents/context/`, gibt Violations als strukturierte Liste aus (exit-code 0 ok, 1 violations)
- [ ] Step 3: `templates/hooks/context-monitor.sh` (existierender Hook) erweitern: ruft size-check auf, bei violation → Warning im Hook-Output mit konkreter Datei + Zeilenzahl + Cap
- [ ] Step 4: `.claude/scripts/doctor.sh` ruft context-size-check auf, zeigt Violations als WARNING
- [ ] Step 5: `context-refresher` Agent (in `.claude/agents/context-refresher.md`) anweisen, Cap-Policy zu respektieren beim Regen: lieber in neue Datei spillen als bestehende überfüllen
- [ ] Step 6: `--relax-context-caps` Flag für Setup-Override (Edge-Case: sehr große Legacy-Repos mit riesigem Stack-Spread)
- [ ] Step 7: Smoke-Test: künstlich SUMMARY.md auf 80 Zeilen aufblasen, Hook + doctor meldet Violation
- [ ] Step 8: Readme-Section "Context File Budget"

## Acceptance Criteria
- [ ] `bash lib/context-size-check.sh` in npx-ai-setup selbst passiert ohne Violations (Meta-Check)
- [ ] Absichtlich überfüllte SUMMARY.md (80 Zeilen) → `lib/context-size-check.sh` exit 1 mit klarer Meldung
- [ ] context-monitor Hook bei überfüllter Datei: Output enthält "Cap exceeded: SUMMARY.md 80 > 40"
- [ ] `doctor.sh` zeigt Violations als WARNING (gelb/orange), nicht als FAIL
- [ ] `--relax-context-caps` unterdrückt Violations ohne die Caps-File zu ändern
- [ ] `shellcheck lib/context-size-check.sh` passt
