...
> **Spec ID**: 102 | **Created**: 2026-03-16 | **Status**: draft | **Branch**: —
...
## Steps
- [ ] Step 1: Update `templates/agents/code-reviewer.md` — change output format to include a confidence score per finding (`[HIGH:92]`, `[MEDIUM:81]`); add rule: suppress findings with score < 80
- [ ] Step 2: Update `templates/agents/code-reviewer.md` verdict logic — FAIL requires HIGH finding with score ≥ 80; CONCERNS requires MEDIUM with score ≥ 80; PASS if nothing clears the threshold

## Acceptance Criteria
- [ ] Each finding in the output includes a numeric score: `[HIGH:92] file:line — description`
- [ ] Findings with confidence < 80 are not reported (suppressed silently)
- [ ] Verdict logic correctly reflects the 80+ threshold
- [ ] Agent template diff is under 15 lines
